...
$CRITERIA = [
    'voice_tone_quality' => 30,
    'mastery_and_timing' => 25,
    'vocal_expression' => 15,
    'diction' => 10,
    'stage_presence' => 10,
    'entertainment_value' => 10
];
...
foreach ($rows as $row) {
    ...
    $subjectId = $row['cand_id'] ?? null;
    if ($subjectId === null || $subjectId === '' || !is_numeric($subjectId)) {
        json_out(422, ['message' => 'Each score needs a valid cand_id.']);
    }

    $entry = ['cand_id' => (int)$subjectId];
...
$subjectIds = array_map(fn($e) => $e['cand_id'], $clean);
...
$already = mysqli_query(
    $conn,
    "SELECT cand_id FROM vocal_score WHERE judge_id = '$safeJudge' AND cand_id IN ($idList)"
);
...
try {
    ...
    foreach ($clean as $entry) {
        do {
            ...
            $check = mysqli_query($conn, "SELECT score_id FROM vocal_score WHERE score_id = '$scoreId'");
        } while ($check && mysqli_num_rows($check) > 0);

        $columns = ['score_id', 'cand_id', 'judge_id'];
        $values = ["'$scoreId'", "'" . (int)$entry['cand_id'] . "'", "'$safeJudge'"];
        foreach ($CRITERIA as $field => $max) {
            $columns[] = $field;
        ...
        $insert = "INSERT INTO vocal_score (" . implode(',', $columns) . ")
                   VALUES (" . implode(',', $values) . ")";
        if (!mysqli_query($conn, $insert)) {
            throw new Exception(mysqli_error($conn));
        }

        $results[] = ['cand_id' => (int)$entry['cand_id'], 'score_id' => $scoreId];
    }
    ...
    foreach ($subjectIds as $subjectId) {
        ...
        $avgResult = mysqli_query(
            $conn,
            "SELECT AVG(total_score) AS avg_total FROM vocal_score WHERE cand_id = '$sid'"
        );
        if (!$avgResult) throw new Exception(mysqli_error($conn));
        $avg = (float)(mysqli_fetch_assoc($avgResult)['avg_total'] ?? 0);

        $exists = mysqli_query($conn, "SELECT cand_id FROM vocal_final_score WHERE cand_id = '$sid'");
        if (!$exists) throw new Exception(mysqli_error($conn));

        $upsert = mysqli_num_rows($exists) > 0
            ? "UPDATE vocal_final_score SET final_score = '$avg' WHERE cand_id = '$sid'"
            : "INSERT INTO vocal_final_score (cand_id, final_score) VALUES ('$sid', '$avg')";
...
json_out(201, [
    'message' => 'Vocal scores submitted successfully',
    'results' => $results,
    'has_submitted' => true,
]);
