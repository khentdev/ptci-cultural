...
$CRITERIA = [
    'originality' => 25,
    'mastery_of_steps' => 15,
    'choreography_and_style' => 20,
    'costume_and_props' => 25,
    'stage_presence' => 15
];
...
$already = mysqli_query(
    $conn,
    "SELECT team_id FROM interpretative_score WHERE judge_id = '$safeJudge' AND team_id IN ($idList)"
);
...
try {
    ...
    foreach ($clean as $entry) {
        do {
            ...
            $check = mysqli_query($conn, "SELECT score_id FROM interpretative_score WHERE score_id = '$scoreId'");
        ...
        $insert = "INSERT INTO interpretative_score (" . implode(',', $columns) . ")
    ...
    foreach ($subjectIds as $subjectId) {
        ...
        $avgResult = mysqli_query(
            $conn,
            "SELECT AVG(total_score) AS avg_total FROM interpretative_score WHERE team_id = '$sid'"
        );
        if (!$avgResult) throw new Exception(mysqli_error($conn));
        $avg = (float)(mysqli_fetch_assoc($avgResult)['avg_total'] ?? 0);

        $exists = mysqli_query($conn, "SELECT team_id FROM interpretative_final_score WHERE team_id = '$sid'");
        if (!$exists) throw new Exception(mysqli_error($conn));

        $upsert = mysqli_num_rows($exists) > 0
            ? "UPDATE interpretative_final_score SET final_score = '$avg' WHERE team_id = '$sid'"
            : "INSERT INTO interpretative_final_score (team_id, final_score) VALUES ('$sid', '$avg')";
...
json_out(201, [
    'message' => 'Interpretative Dance scores submitted successfully',
    'results' => $results,
    'has_submitted' => true,
]);
