<template>
  <ScoreboardPage :is-pending="store.finalScores.isPending" :is-fetching="store.finalScores.isFetching"
    ...
    <FeatureHeader :title="`${category.label} - Top 3`"
      :description="`The three highest averaged ${subjectNoun} scores.`" />
    <OverallScoreDataTable :category="category" :rows="topThree" :retry-fn="store.refetchFinalScores"
...
<script setup lang="ts">
  ...
  // Top 3 is a slice of the same "final" query the Final Scores page uses, so it
  // shares that cache and costs no extra request.
  const { topThree, finalFetchError } = storeToRefs(store);
