<template>
    <feature-base-table>
        <template #table>
            <feature-server-state :on-retry="retryFn" :title="`Couldn't load modern contestants`"
            ...
            <table :class="TABLE_STYLES.TB" v-else>
                <thead>
                    <tr>
                        ...
                        <th :class="TABLE_STYLES.TH">Audience Impact (10%)</th>
                        <th :class="TABLE_STYLES.TH">Mastery of Steps (25%)</th>
                        <th :class="TABLE_STYLES.TH">Choreography and Style (30%)</th>
                        <th :class="TABLE_STYLES.TH">Costume and Props (20%)</th>
...
<script lang="ts" setup>
    ...
    const { getModernTeams, isSubmitted } = useModernStore()
    ...
    const SCORE_CRITERIA = {
        audience_impact: { max: 10 },
        mastery_of_steps: { max: 25 },
        choreography_and_style: { max: 30 },
        costume_and_props: { max: 20 },
        stage_presence: { max: 15 },
    } as const;

    const candidatesQuery = computed(() => getModernTeams)
    ...
    const validateFields = () => {
        const hasError = candidateScoreInputs.value.some((c) => {
            return !c.audience_impact || !c.mastery_of_steps || !c.choreography_and_style || !c.costume_and_props || !c.stage_presence
        });
        return hasError;
    };
