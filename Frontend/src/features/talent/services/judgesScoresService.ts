...
export const judgesScoresService = {
    getJudgesScoresMale: async () => {
        const res = await axiosInstance.get(`/category/talent/getJudgeScore.php?gender=male`)
        return GetTypedResponse<JudgeScoresMalesResponse>(res)
    },
    getJudgesScoresFemale: async () => {
        const res = await axiosInstance.get(`/category/talent/getJudgeScore.php?gender=female`)
