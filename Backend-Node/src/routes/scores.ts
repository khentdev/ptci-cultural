...
import { judgeScoresGrouped, myJudgeScores, subjectFinals } from '../services/scoreboardService.js'
...
export const scoreRoutes: FastifyPluginAsync = async (app) => {
  ...
  // POST /api/scores/:category  { <cand_id|team_id>, ...criteria }  (judge)
  //   → 200 { status, message, score_id, total_score, has_submitted }
  //   → 404 unknown category/subject · 422 invalid value or already scored
  app.post('/scores/:category', { preHandler: [authenticate] }, async (request) => {
    ...
    const idColumn = CATEGORIES[category].subject.idColumn
    const subjectId = (request.body as Record<string, unknown> | undefined)?.[idColumn]
    logActivity(request, 'score.submit', `${category} ${idColumn}=${subjectId} total=${result.total_score}`)
    return result
  })

  // POST /api/scores/:category/batch  [{ <cand_id|team_id>, ...criteria }, ...]  (judge)
  ...
  //   → 404 unknown category/subject · 422 invalid value or already scored (rolls back everything)
  ...
  // GET /api/scores/:category/mine → { data: rows[] } — the current user's own submitted rows.
  // The judge screens use this to keep inputs locked across a refresh or a new device.
  app.get('/scores/:category/mine', { preHandler: [authenticate] }, async (request) => {
    const category = categoryParam(request.params)
    const data = await myJudgeScores(category, request.user!.id)
    return { status: 200, message: `${CATEGORIES[category].label} scores fetched successfully.`, data }
  })

  // GET /api/scores/:category/judges → { data: { "<judge_id>": rows[] } } (admin)
  app.get('/scores/:category/judges', { preHandler: [requireRole('admin')] }, async (request) => {
    const category = categoryParam(request.params)
    const data = await judgeScoresGrouped(category)
    return { status: 200, message: `${CATEGORIES[category].label} judge scores fetched successfully.`, data }
  })

  // GET /api/scores/:category/final → per-subject averages, best first (admin)
  app.get('/scores/:category/final', { preHandler: [requireRole('admin')] }, async (request) => {
    const category = categoryParam(request.params)
    const data = await subjectFinals(category)
