...
/**
 * One scores table per category, generated from scoring/categories.ts. The subject
 * column and its foreign key come from the category's SubjectConfig, so a
 * team-scored category and a contestant-scored one share this one generator.
 */
function scoresTableStatement(key: (typeof CATEGORY_KEYS)[number]): string {
  ...
  return `CREATE TABLE IF NOT EXISTS ${cat.table} (
    ...
    ${subject.idColumn} INT UNSIGNED NOT NULL,
${criteriaColumns}
    total_score DECIMAL(6,2) NOT NULL,
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (score_id),
    UNIQUE KEY uq_judge_subject (judge_id, ${subject.idColumn}),
    KEY idx_subject (${subject.idColumn}),
    CONSTRAINT fk_${fk}_judge FOREIGN KEY (judge_id) REFERENCES users (id) ON DELETE RESTRICT,
    CONSTRAINT fk_${fk}_subject FOREIGN KEY (${subject.idColumn})
      REFERENCES ${subject.table} (${subject.idColumn}) ON DELETE CASCADE
  ) ${TABLE_OPTS}`
}
...
export const SCHEMA_STATEMENTS: string[] = [
  ...
  // Teams are the single source of truth: scored directly by both dance
  // categories, and referenced by every contestant.
