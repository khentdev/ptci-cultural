...
> **Cultural Night runs cross-site.** The frontend is deployed on **Vercel**
> (`cultural-night.vercel.app`) while this API sits on a Hostinger subdomain — two
> different sites, so the session cookie needs `SameSite=None; Secure`. The `auto`
> defaults already resolve to exactly that in production, so **leave
> `COOKIE_SAME_SITE` and `COOKIE_SECURE` unset and `COOKIE_DOMAIN` empty**. Do not
> copy the `lax` value from a same-site deployment: login will appear to work and
> then every following request will 401.

```env
NODE_ENV=production
HOST=0.0.0.0
# PORT is injected by Hostinger — don't set it
CORS_ORIGIN=https://cultural-night.vercel.app   # exact origin, NO trailing slash
TRUST_PROXY=auto
MYSQL_HOST=localhost
MYSQL_PORT=3306
MYSQL_USER=u123456_culturaladmin
MYSQL_PASSWORD=********
MYSQL_DATABASE=u123456_cultural
JWT_SECRET=<64+ random chars>
JWT_EXPIRES_IN=1d
COOKIE_NAME=cultural_token
```

A trailing slash on `CORS_ORIGIN` is the exact bug that left the old PHP backend
unreachable from the deployed frontend — an `Origin` header never carries one.

(If the API and frontend are ever moved onto the same domain, set
`COOKIE_SAME_SITE=lax`, which is the safer choice there.)
...
4. Deploy settings Hostinger asks for:
   ...
   | Node.js version | 22 |
   | Build command | `npm run build` — plain `tsc`. (Hostinger runs `npm install` first; the zip already contains `dist/`, so this is just a safety rebuild.) |
